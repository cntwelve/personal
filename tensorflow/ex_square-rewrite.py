...
def main(argv):
    ...
    def input_train():
        return (
            ...
            tf.data.Dataset.from_tensor_slices((dict(train_x), train_y)).shuffle(1000).batch(128)
            # Repeat forever
    ...
    def input_test():
        return (
            # Shuffling with a buffer larger than the data set ensures
            # that the examples are well mixed.
            tf.data.Dataset.from_tensor_slices((dict(test_x), test_y)).shuffle(300).batch(128)
            # Repeat forever
            .repeat().make_one_shot_iterator().get_next())
    ...
    # Train the Model.
    classifier.train(
        input_fn=input_train, steps=1000)
